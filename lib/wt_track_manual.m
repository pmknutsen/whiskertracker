...
if isempty(g_tWT.MovieInfo.SplinePoints)
    ...
    errordlg (sErrString); error(sErrString);
    return
end
...
if strcmp(sOptions, 'without_deletion')
    ...
    if size(g_tWT.MovieInfo.SplinePoints, 3) > nFirstFrame
        sAnsw = questdlg('Existing tracking data after this frame will be deleted.', 'Warning', 'Ok', 'Cancel', 'Cancel');
...
for nFrame = vDoFrames(1):g_tWT.MovieInfo.NoFramesToLoad:vDoFrames(end)
    ...
    for f = 1:size(mFrames, 3)
        ...
        if bHeadIsTracked
            ...
            try
                [mImg, mImgCroppedOnly] = wt_image_preprocess(mImg);
            catch meException
                wt_error(sprintf('Failed pre-processing image: %s', lasterr));
            end
        end
        ...
        if g_tWT.ShowSR
            wt_plot_signal_noise(nScore./nScoreStd)
            g_tWT.StopProc = 0;
        end
        ...
        while 1
            try
                if ~bTipped
                    wt_set_status('Click to select new locations for splinepoints, or Return to quit.');
                    bTipped = 1;
                end
                figure(hFrameWin)
                [nX, nY, nBtn] = ginput(1);  % user may press Return to cancel
                wt_set_status('')
            catch
                nBtn = 2; % no input data, treat as if middle mouse button or Return was hit
            end
            ...
            [~, nIndx] = min(sqrt(abs(vX-nX).^2 + abs(vY-nY).^2)); % locate closest point
