...
% Fields to export
cFields = {'Angle', 'Curvature', 'Intersect', 'PositionOffset', ...
    'AvgWhiskerPos', 'ROIEvent', ...
    'StimulusA', 'StimulusB', ...
    'VP_FFT', 'VP_RealTimeTracking', 'VP_FrameNumber'};
...
for f = 1:length(cFields)
    ...
    elseif any(strcmp(cFields{f}, {'ROIEvent', 'StimulusA', 'StimulusB', ...
            'VP_FFT', 'VP_RealTimeTracking', 'VP_FrameNumber'}))
        % Fields that should be exported as-is, raw with not formatting etc
        if ~isfield(sWT.MovieInfo, cFields{f}), continue, end
        if isempty(sWT.MovieInfo.(cFields{f})), continue, end
        cVarNames{end+1} = cFields{f};
        tVars(1).(cVarNames{end}) = sWT.MovieInfo.(cFields{f});
...
sDefFileName = [sWT.MovieInfo.Filename(1:end-4) '_Whisker_Tracking.mat'];
%uisave(cVarNames, sDefFileName); % interactive save
save(sDefFileName, '-struct', 'tVars'); % default save

wt_set_status(['Exported tracking data of current movie (OK)'])
