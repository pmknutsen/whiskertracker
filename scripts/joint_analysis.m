...
% Shoulder - Elbow
vSE_theta = unwrap(atan2(mE(:,2) - mS(:,2), mE(:,1) - mS(:,1)));
vSE_rho = sqrt((mE(:,2) - mS(:,2)).^2 + (mE(:,1) - mS(:,1)).^2);

% Elbow - Wrist
vEW_theta = unwrap(atan2(mW(:,2) - mE(:,2), mW(:,1) - mE(:,1)));
vEW_rho = sqrt((mW(:,2) - mE(:,2)).^2 + (mW(:,1) - mE(:,1)).^2);

% Wrist - Digit
vWD_theta = unwrap(atan2(mD(:,2) - mW(:,2), mD(:,1) - mW(:,1)));
...
vW_theta = vWD_theta - vEW_theta;

% Plot joint angles
hAx(2) = subplot(2,2,[3 4]);
hold(hAx(2), 'on')
plot(hAx(2), rad2deg(vA_theta - vA_theta(1)), 'r')
plot(hAx(2), rad2deg(vW_theta - vW_theta(1)), 'g')
plot(hAx(2), rad2deg(vSE_theta - vSE_theta(1)), 'b')

%plot(hAx(2), rad2deg(vEW_theta), 'k')
%plot(hAx(2), rad2deg(vWD_theta), 'k')
xlabel(hAx(2), 'Time (frames)')
ylabel(hAx(2), 'Angle (flexion - extension; deg)')

legend({'Arm', 'Wrist', 'Shoulder'})
legend boxoff
axis(hAx(2), 'tight')
